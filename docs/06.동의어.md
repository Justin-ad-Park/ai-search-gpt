...
4. 서버가 `POST /{index}/_reload_search_analyzers` 호출
...
### 중요
- `mode=REGRESSION` 자체가 ES에 저장되지는 않습니다.
...
6. `POST /{index}/_reload_search_analyzers` 실행
7. 이후 검색 쿼리부터 반영
...
> 이어야 합니다.
>
> 순서가 잘못되면 동의어 파싱이 깨져 샤드가 올라오지 않을 수 있습니다.
...
### 핵심 교훈
- 포트포워딩/타임아웃으로 보이더라도, 실제 원인은 analyzer 체인 순서일 수 있습니다.
