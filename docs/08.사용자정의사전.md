...
번호 순서 실행 기준:
1. `./sh_bin/setup/01_delete_elasticsearch_resources.sh` (필요 시)
2. `./sh_bin/setup/02_install_eck_operator.sh`
3. `./sh_bin/check/01_check_eck_operator_status.sh`
4. `./sh_bin/setup/03_build_elasticsearch_nori_image.sh`
5. `./sh_bin/setup/04_push_elasticsearch_nori_image.sh` (필요 시)
6. `.local-nas/userdict/user_dict_ko.txt` 준비
6. `./sh_bin/setup/05_start_elasticsearch_cluster_custom_image.sh`
7. `./sh_bin/check/02_check_elasticsearch_nori_plugin.sh`
...
2. `indexing` 프로파일 또는 [IndexRolloutService.java](/Users/justinpark/idea/ai_search/ai-search-gpt/src/main/java/com/example/aisearch/service/indexing/orchestration/IndexRolloutService.java) 경로로 재색인 + alias 전환
3. `_analyze` 또는 dictionary 통합 테스트로 기대 토큰 확인
...
- 해결: active 사전 파일 교체 후 재색인 + alias 전환까지 수행
...
이 프로젝트에서 사용자 정의 사전은  
**파일 기반으로 관리하고, 재색인/검증 절차를 표준화해서 운영**합니다.
