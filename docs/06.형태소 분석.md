...
예시 `_analyze` (권장 스크립트):
```bash
 ./sh_bin/analyze/01_nori_analyse.sh "얇은피 만두"
{
  "tokens": [
    {
      "token": "만두",
...
./sh_bin/analyze/01_nori_analyse.sh "아버지가방에들어가십니다."  
...
./sh_bin/analyze/01_nori_analyse.sh "아버지가 방에 들어가십니다."
...
아래는 `./sh_bin/analyze/01_nori_analyse.sh` 실행 결과를 간략히 비교한 표입니다.
