...
다음과 같은 의미입니다:
- `s`는 **ES `SearchRequest.Builder`** 역할을 하는 빌더
- `s -> ...` 람다는 **ES `SearchRequest.Builder`를 어떻게 채울지** 알려주는 함수
...
### 2) `s -> s.index(...)`
- `s`는 **ES `SearchRequest.Builder`** 역할.
- `.index(...)`는 검색할 **인덱스 이름**을 지정한다.

### 3) `.knn(knn -> ...)`
- 이 요청은 <font color="#FFD700"><b>벡터 검색(kNN 검색)</b></font>을 수행한다.
...
### 6) `.k((long) size)`
- 최종 결과로 <font color="#FFD700"><b>가져올 문서 개수</b></font>(Top-K).
- `size=5`면 가장 유사한 5개 문서.

### 7) `.numCandidates(numCandidates)`
- 검색할 때 **후보군으로 평가할 문서 수**.
- <font color="green"><b>`k`보다 크게 잡아야 검색 품질이 좋아짐.</b></font>
...
- <font color="red"><b>`numCandidates`를 너무 작게 잡으면 관련 문서를 놓칠 수 있다.</b></font>
