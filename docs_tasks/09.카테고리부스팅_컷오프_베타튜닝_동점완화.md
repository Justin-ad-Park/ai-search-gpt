...
1. `minScoreThreshold` 미만 문서가 부스팅으로 노출되지 않게 차단
...
- 컷오프 기준값은 기존 `ai-search.min-score-threshold` 하나만 사용한다. (현재 기본값 0.74)
...
## 1) cutoff 결과 제거 (`minScoreThreshold` 미만 제외)
...
- 컷오프 기준을 `minScoreThreshold` 하나로 일관 유지
...
```java
.minScore(properties.minScoreThreshold()) // 현재 기본값 0.74
```
...
참고: TODO 문서에는 구현 코드를 고정하지 않고, 위 시그니처/제약만 확정한다.
...
- `categoryBoostBetaTuner.setBeta(0)`에서 카테고리 부스팅 영향 제거 확인
  - 주석: `beta=0`이면 기존 부스트 룰 영향은 0이어야 함
