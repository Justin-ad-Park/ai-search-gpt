...
4. `POST /{index}/_reload_search_analyzers` 호출
5. 결과 응답 반환
...
- `index`, `synonymsSet` 미입력 시 서버 기본값 사용 가능
...
- `500 Internal Server Error`
  ...
  - 예시 `message`: `failed to reload analyzers for index ai-search-products`
...
3. 생만두 상품 노출 여부 확인
- 검증 포인트:
1. 동의어 규칙 `만두, 떡국`이 반영되어 `q=떡국`에서 생만두가 검색되는지
...
2. 회귀 테스트용 규칙(`만두, 떡국`)이 운영 검색에 남지 않는지
