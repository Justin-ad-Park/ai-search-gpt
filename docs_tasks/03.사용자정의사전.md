...
주의:
- `configMap.name`은 `00_5_apply_user_dictionary_configmap.sh`의 기본값 `es-user-dict`와 맞춰야 한다.
...
## 6. 운영 절차 (단순 버전)
1. 사전 수정
2. ConfigMap 갱신 (`./sh_bin/00_5_apply_user_dictionary_configmap.sh`)
3. ES Pod 롤링 재시작
4. 인덱스 롤아웃 실행
5. 대표 쿼리 검증

운영 정책(현재 프로젝트):
- 현재 ECK 구성은 단일 노드(`count: 1`) 기준이므로 롤링 재시작 동안 짧은 검색 장애를 감수한다.
...
3. 검증 스크립트 실행
```bash
./sh_bin/05_2_veryfyUserDictionary.sh
```

`05_1_reloadUserDictionary.sh`가 수행하는 작업:
- `00_5_apply_user_dictionary_configmap.sh` 실행
- ES Pod 롤링 재시작 + 상태 대기
- `indexing` 프로파일로 재색인 + alias 전환
- ES Pod 내부 사용자 사전 파일 마운트 존재 확인

`05_2_veryfyUserDictionary.sh`가 수행하는 작업:
...
- 현재 단일 노드 구성에서는 롤링 재시작 중 짧은 검색 장애가 발생할 수 있다.
- 단, 최초 1회는 `00_6_start_elasticsearch_cluster_custom_image.sh`로 user-dict 마운트 포함 CR이 적용되어 있어야 한다.
