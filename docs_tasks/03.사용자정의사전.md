...
초기 구축(매핑 반영 + YAML 마운트 반영)이 끝난 이후에는 아래 절차로 반복 운영한다.
...
3. `_analyze` 또는 dictionary 통합 테스트로 기대 토큰을 확인
...
- 단, 최초 1회는 `./sh_bin/setup/05_start_elasticsearch_cluster_custom_image.sh`로 userdict PV/PVC 마운트 포함 CR이 적용되어 있어야 한다.
