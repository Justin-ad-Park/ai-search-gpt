...
현재 프로젝트는 커스텀 이미지 흐름을 사용 중이므로, 사용자 사전 파일이 컨테이너 경로에 포함되도록 유지한다.

### 2.4 ES 내부 파일 경로 주의
- `src/main/resources/...` 경로는 **애플리케이션 리소스** 경로다.
- `user_dictionary` 설정값은 **Elasticsearch 컨테이너 내부 경로** 기준이어야 한다.
- 예시: `analysis/user_dict_ko.txt`
- 따라서 커스텀 이미지 빌드 시 해당 파일이 ES 컨테이너의 기대 경로에 복사되도록 Dockerfile/manifest를 맞춰야 한다.

## 2.5 sh_bin 실행 순서 (프로젝트 기준)
사용자 사전 반영이 필요한 Nori 커스텀 이미지 흐름:
...
참고:
- 로컬 Docker Desktop만 쓰면 `00_4`는 스킵 가능(로컬 이미지 직접 사용)
...
2. 신규 인덱스 생성 + 재색인
3. 동의어 규칙이 필요한 경우 `reload-synonyms` API 호출
4. 대표 키워드 검색으로 품질 확인
5. alias 전환 및 모니터링
