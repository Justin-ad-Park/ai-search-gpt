...
### 2.2 현재 구조의 리스크
- `KnnSearchStrategy#vectorScoreSearch` 내부에 카테고리 부스팅 분기까지 직접 추가하면 조건문이 빠르게 증가할 수 있다.
...
작업:
- 기존 `vectorScoreSearch`에서 직접 분기하지 말고 `CategoryBoostingDecision` 주입받아 실행
- script params 구성 시:
  - 항상 `query_vector` 주입
...
2. `sort=CATEGORY_BOOSTING_DESC`, `q=abc` -> `RELEVANCE_DESC`와 동일 순서 확인
3. `sort=PRICE_ASC`, `q=간식` -> 가격 오름차순 회귀 없음

### 7.3 카테고리 부스팅 테스트 케이스 작업 (필수)
아래 3개는 구현 완료 전 반드시 테스트 코드로 추가한다.

1. 카테고리 부스팅을 사용하지 않는 기존 정렬 테스트
- 대상: `RELEVANCE_DESC`, `PRICE_ASC`, `PRICE_DESC`
- 목적: 카테고리 부스팅 기능 추가 이후에도 기존 정렬 결과가 기존과 동일하게 유지되는지 회귀 검증

2. 카테고리 부스팅 키워드가 일치할 때 특정 카테고리 상품이 상위 노출되는 테스트
- 예시: `sort=CATEGORY_BOOSTING_DESC`, `q=사과` -> `categoryId=4(과일)` 상품이 상위권에 존재
- 예시: `sort=CATEGORY_BOOSTING_DESC`, `q=오징어` -> `categoryId=7(수산)` 상품이 상위권에 존재
- 목적: `equals` 일치 시에만 boost score가 실제 정렬에 반영되는지 검증

3. 카테고리 부스팅 키워드가 일치하지 않을 때 부스팅 없이 정렬되는 테스트
- 예시: `sort=CATEGORY_BOOSTING_DESC`, `q=오징어 튀김`
- 기대: 내부 fallback(`RELEVANCE_DESC`)가 적용되어, 결과 순서가 `sort=RELEVANCE_DESC`, `q=오징어 튀김`와 동일
- 목적: 불일치 키워드에서 불필요한 가산점/분기 오동작이 없는지 검증
