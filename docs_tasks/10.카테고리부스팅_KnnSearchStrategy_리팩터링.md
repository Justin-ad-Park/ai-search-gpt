...
### 타입명 명확화 원칙
- 앱 요청 모델은 `com.example.aisearch.model.search.ProductSearchRequest`를 `searchRequest`로 명명
...
### 제공 메서드
```java
Query build(com.example.aisearch.model.search.ProductSearchRequest searchRequest, Optional<Query> filterQuery)
```
...
- `integration/synonyms/SynonymReloadApiIntegrationTest`
- `integration/synonyms/SynonymsSearchSmokeIntegrationTest`
