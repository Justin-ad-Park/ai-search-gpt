...
권장 설계:
1. 버전 조회 인터페이스 분리
- 예: `CategoryBoostVersionSource#getVersion()`
- 운영 구현: JSON `version` 필드 조회
- 테스트 구현: 메모리 값 변경 가능한 Fake 구현

2. 룰 로딩 인터페이스 분리
- 예: `CategoryBoostRuleLoader#loadAll()`
- 테스트에서 호출 횟수 카운터를 두어 실제 재로딩 여부 검증
...
## 12. 로컬 수동 검증 스크립트 가이드
### 12.1 스크립트 경로 제안
- `sh_bin/06_1_set_category_boost_version.sh`

### 12.2 동작 방식
- 입력 인자: `newVersion`
- 대상 파일: `src/main/resources/data/category_boosting.json`
- 수행:
  1. 현재 파일을 동일 경로 `.bak`으로 백업
  2. JSON의 `version` 값을 `newVersion`으로 변경
  3. 변경된 버전 출력

원복:
- 테스트 후 `.bak` 파일로 자동/수동 복원
- 복원 스크립트(선택): `sh_bin/06_2_restore_category_boost_version.sh`

주의:
- 이 방식은 로컬 수동 검증용이며, 자동 테스트(특히 CI)에서는 권장하지 않는다.
- CI에서는 mock/fake 기반 버전 변경 가정 테스트를 기본으로 사용한다.

---

## 13. 참고 파일
- `src/main/java/com/example/aisearch/service/search/strategy/KnnSearchStrategy.java`
- `src/main/java/com/example/aisearch/service/search/boosting/*` (리팩터링 대상)
