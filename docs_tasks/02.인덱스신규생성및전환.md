...
- 구현 포인트:
  - `IndexManagementService#createVersionedIndex()`에서 인덱스명 생성 + 인덱스 생성까지 책임진다.
...
- 구현 포인트:
  - `ProductIndexingService#indexSourceData(newIndex)`로 명시적 대상 인덱스를 받아 색인한다.
...
- 구현 포인트:
  - `AliasService#swapReadAlias(oldIndex, newIndex)`로 원자 전환한다.
...
- 구현 포인트:
  - `IndexManagementService#deleteIndex(oldIndex)`를 별도 단계로 호출한다.
  - 삭제는 즉시가 아니라 안정화 확인 후 실행한다.

## 5. 롤백 절차
- alias 전환 후 문제 발생 시 즉시 원복한다.
- 구현 포인트:
  - `AliasService#rollbackReadAlias(newIndex, oldIndex)`를 통해 즉시 원복한다.
  - 원복 후 `newIndex`는 원인 분석 전까지 삭제하지 않는다.

## 6. 애플리케이션 로직 개선 포인트 (IndexingRunner)
- 현재:
```java
...
- 개선 방향:
  ...
  - 아래 순서의 오케스트레이션 메서드로 변경한다.
    1. `createVersionedIndex()` : `-v{yyyyMMddHHmmss}` 인덱스 생성
    2. `indexSourceData(newIndex)` : 원천 데이터 색인
    3. `swapReadAlias(oldIndex, newIndex)` : alias 원자 전환
    4. `deleteOldIndex(oldIndex)` : 안정화 후 구 인덱스 삭제
- 핵심:
  - 검색은 항상 alias 기준
  - 색인 완료 전까지 기존 인덱스로 서비스 지속
  - 색인 실패 시 alias 미전환으로 안전 보장

## 7. 테스트 영향 범위 및 방식 (VectorSearchIntegrationTest)
- 목적:
...
- 테스트 방식:
  ...
  3. `VectorSearchIntegrationTest`의 기존 검색 시나리오를 alias 기준으로 실행
