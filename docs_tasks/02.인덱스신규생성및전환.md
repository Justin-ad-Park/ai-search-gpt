...
- 구현 포인트:
  - `IndexManagementService#swapReadAlias(oldIndex, newIndex)`로 원자 전환한다.
...
- 구현 포인트:
  - `IndexManagementService#deleteIndexIfExists(oldIndex)`를 별도 단계로 호출한다.
  - 삭제는 즉시가 아니라 안정화 확인 후 실행한다.

## 5. 롤백 절차
- alias 전환 후 문제 발생 시 즉시 원복한다.
- 구현 포인트:
  - `IndexManagementService#swapReadAlias(newIndex, oldIndex)`를 통해 즉시 원복한다.
