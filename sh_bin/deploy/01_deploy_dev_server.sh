...
usage() {
  cat <<'EOF'
[USAGE] ./sh_bin/deploy/01_deploy_dev_server.sh [--models <all|comma-separated>] [--skip-test] [--skip-index]

Examples
  ./sh_bin/deploy/01_deploy_dev_server.sh
  ./sh_bin/deploy/01_deploy_dev_server.sh --models e5-small-ko-v2
  ./sh_bin/deploy/01_deploy_dev_server.sh --models e5-small-ko-v2,kure-v1 --skip-test
  ./sh_bin/deploy/01_deploy_dev_server.sh --models all --skip-index
EOF
}
...
start_model_web() {
  ...
  OPTIONAL_PROFILE="search-vector-only" "${ROOT_DIR}/sh_bin/model/02_run_model_web.sh" "${model}" > "${log_file}" 2>&1 &
  echo $! > "${pid_file}"
...
"${ROOT_DIR}/sh_bin/model/07_stop_all_models.sh"

if [ "${RUN_INDEXING}" = true ]; then
  for model in "${TARGET_MODELS[@]}"; do
    echo "[STEP] reindex ${model}"
    "${ROOT_DIR}/sh_bin/model/04_run_model_indexing_only.sh" "${model}"
  done
else
...
"${ROOT_DIR}/sh_bin/check/05_check_model_status.sh"
