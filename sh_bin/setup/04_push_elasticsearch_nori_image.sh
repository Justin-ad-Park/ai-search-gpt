...
if ! docker image inspect "${SOURCE_IMAGE}" >/dev/null 2>&1; then
  ...
  echo "[INFO] push 단계를 건너뜁니다. 필요하면 먼저 빌드하세요: ./sh_bin/setup/03_build_elasticsearch_nori_image.sh"
  exit 0
fi
...
if [ -z "${TARGET_IMAGE}" ]; then
  ...
  echo "[NEXT] 로컬 이미지를 그대로 사용해 다음 단계를 실행하세요: ./sh_bin/setup/05_start_elasticsearch_cluster_custom_image.sh"
  exit 0
fi
