...
#
# 사용 예시
#   ./sh_bin/analyze/02_nori_analyse.sh "얇은피 만두"
#   ANALYZE_MODE=analyzer ANALYZER_NAME=ko_mall_search_analyzer ./sh_bin/analyze/02_nori_analyse.sh "얇은피 만두"
#   ANALYZE_MODE=tokenizer TOKENIZER_NAME=ko_nori_userdict_tokenizer TOKEN_FILTERS=lowercase,nori_part_of_speech ./sh_bin/analyze/02_nori_analyse.sh "얇은피 만두"
...
if [ -z "${ANALYZE_TEXT}" ]; then
  ...
  echo "[예시] ./sh_bin/analyze/02_nori_analyse.sh '얇은피 만두'"
  exit 1
fi
